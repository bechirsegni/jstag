...
describe("testing the jstag.send and jstag.mock(wrapper) functions", function () {
  ...
  it("should send four calls with correct data as payload", function () {
      ...
      expect(async1.dataMsg).toEqual('one=one&_ts=' + async1.data._ts + '&_nmob=t&_device=desktop&url=localhost%3A9876%2Fcontext.html&_if=t&_v=1.31&_e=pv&_sesstart=1&_tz=-7&_ul=en-US&_sz=1024x768&_uid=' + async1.data._uid + '&_getid=t&_ca=jstag1');

      // future sends should not include session but do include new _uid
      expect(async2.dataMsg).toEqual('two=one&_ts=' + async2.data._ts + '&_nmob=t&_device=desktop&url=localhost%3A9876%2Fcontext.html&_if=t&_uid=' + async2.data._uid + '&_getid=t&_v=1.31&_ca=jstag1');
      expect(async3.dataMsg).toEqual('three=one&_ts=' + async3.data._ts + '&_nmob=t&_device=desktop&url=localhost%3A9876%2Fcontext.html&_if=t&_uid=' + async3.data._uid + '&_getid=t&_v=1.31&_ca=jstag1');
      expect(async4.dataMsg).toEqual('four=one&_ts=' + async4.data._ts + '&_nmob=t&_device=desktop&url=localhost%3A9876%2Fcontext.html&_if=t&_uid=' + async4.data._uid + '&_getid=t&_v=1.31&_ca=jstag1');
  });
});
...
describe("testing that jstag.send can handle an array in addition to an object", function () {
  ...
  it("should translate the array into key/value pairs", function () {
    expect(async1.dataMsg).toEqual('0.one=one&1.two=two&_ts=' + async1.data._ts + '&_nmob=t&_device=desktop&url=localhost%3A9876%2Fcontext.html&_if=t&_uid=' + async1.data._uid + '&_getid=t&_v=1.31&_ca=jstag1');
  });
});
...
describe("verify that we can properly process a pageview event", function () {
  ...
  it("should add and/or alter the _e param and start session", function () {
    expect(async1.dataMsg).toEqual('_e=pv&_sesstart=1&_tz=-7&_ul=en-US&_sz=1024x768&_ts=' + async1.data._ts + '&_nmob=t&_device=desktop&url=localhost%3A9876%2Fcontext.html&_if=t&_uid=' + async1.data._uid + '&_getid=t&_v=1.31&_ca=jstag1');
  });
...
describe("verify that we can properly process an identify event", function () {
  ...
  it("should add the user_id param to identify the user", function () {
    ...
    expect(async1.dataMsg).toEqual('user_id=myfakeuserid&_ts=' + async1.data._ts + '&_nmob=t&_device=desktop&url=localhost%3A9876%2Fcontext.html&_if=t&_uid=' + async1.data._uid + '&_getid=t&_v=1.31&_ca=jstag1');
  });
});
...
describe("verify that we can block all sends and then flush the queue to handle state aware entity lookup", function() {
  ...
  it("should not fill up the queue when the block flag is set to false", function() {
    ...
    expect(async1.dataMsg).toEqual('test=test&_ts=' + async1.data._ts + '&_nmob=t&_device=desktop&url=localhost%3A9876%2Fcontext.html&_if=t&_uid=' + async1.data._uid + '&_getid=t&_v=1.31&_ca=jstag1');
  });
});
...
describe("verify that events fire after they have been added to queue by block", function() {
  ...
  it("should receive outbound query params for previously blocked sends", function() {
    expect(async1.dataMsg).toEqual('test=test&_ts=' + async1.data._ts + '&_nmob=t&_device=desktop&url=localhost%3A9876%2Fcontext.html&_if=t&_uid=' + async1.data._uid + '&_getid=t&_v=1.31&_ca=jstag1');
    expect(async2.dataMsg).toEqual('test2=test2&_ts=' + async2.data._ts + '&_nmob=t&_device=desktop&url=localhost%3A9876%2Fcontext.html&_if=t&_uid=' + async2.data._uid + '&_getid=t&_v=1.31&_ca=jstag1');
  });
});
...
describe("verify that all blocked events in the queue are added the the payload of the mock call", function() {
  ...
  it("should merge the payload from the two blocked events and one mock event", function() {
    expect(async1.dataMsg).toEqual('test=test&test2=test2&test3=test3&_ts=' + async1.data._ts + '&_nmob=t&_device=desktop&url=localhost%3A9876%2Fcontext.html&_if=t&_uid=' + async1.data._uid + '&_getid=t&_v=1.31&_ca=jstag1');
  });
});
