describe("verify that we can handle an array of cid", function() {
  ...
  function testSend(done) {
    jstag.pageView("test", {"one":"value1"}, function(opts, self){
      async1 = opts;
      count++;
      if(count >= 2){
        done();
  ...
  it("should send the same data to two accounts", function () {
    expect(async1.sendurl).toEqual(['//localhost:3001/c/bogusaccountid2/test', '//localhost:3001/c/bogusaccountid/test']);
